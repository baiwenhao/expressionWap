<template>
  <div class="material">
    ...
    <div class="menu flex">
      <div @click="setMenu(v)" v-for="v in menu" :class="{ 'active': v.active }">{{ v.name }}</div>
    </div>
    <div class="list_item" :class="[menuType === 'menuRecent' ? 'four' : 'three']">
      ...
      <div class="flex" v-else>
        ...
        <div class="f1 scroll">
          ...
          <div class="_copyright">Copyright© 来源于网络</div>
        </div>
      </div>
    </div>
...
<script>
  ...
  import Images from '../components/ImagesCms'
    ...
    methods: {
      ...
      setMenu (v) {
        ...
        if (v.type === 'menuRecent') {
          const d = getData('recent_expression')
      ...
      loadMain (v) {
        ...
        if (d) {
          ...
          this.$refs.imgs.setMore('end', false)
          this.$refs.imgs.setMore('more', false)
          this.detail.page = 0
        ...
        axios('img', 'get', params).then((res) => {
          if (res.status) {
            if (res.data.length) {
              ...
              this.$refs.imgs.setMore('end', true)
            }
            ...
            this.changeDetailStatus(true)
            if (this.detail.templetName === '热门') {
              this.$refs.imgs.setMore('more', true)
            } else {
              this.$refs.imgs.setMore('more', false)
            }
          } else {
