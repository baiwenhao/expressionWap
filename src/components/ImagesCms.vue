      ...
      <div class="min_h">
        ...
        <div class="flex" style="padding: 24px 18px;padding-bottom: 0;">
          <div @click="addImg(v)" class="item" v-for="v in list">
            ...
            <div v-show="v.createDetail" class="view"></div>
          </div>
        </div>
      </div>
      <div v-show="end" style="text-align: center;">没有更多了</div>
  ...
  export default {
    ...
    methods: {
      resize (e) {
        ...
        if (e.target.offsetHeight + top >= e.target.scrollHeight && !this.more && !this.end) {
          this.$emit('load')
          this.more = true
        }
